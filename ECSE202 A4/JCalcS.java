...
public class JCalcS extends ConsoleProgram{
	...
	private Boolean isNumber (String input) {
		...
		for (int i= 0; i<input.length(); i++) {							//go through each index of the string
			char test = input.charAt(i);
			if (test == '.') {											//if the index is ".", check the length of the string and the number of "." detected
			...
			}else if (!Character.isDigit(test)) {						//if the index is not number, return false
				return false;
			}
		}
	 ...
	 * 
	 * */
	private Boolean onlyLetter (String input) {
	...
	public void run() {
		...
		while(true) {
			...
			int stepcount = 0;								//this int is used to count the number of steps
		
			String formula = readLine("Enter expression (blank line to exit): ");			//ask for the user input
			
			if (formula.equals("")) {										//if the user input is blank, break the while loop and terminate the program
				print("Blank line detected...Program terminated...");
				break;
			}
			...
			while(input.front != null && error == false) {													//keep the loop until there is no more elements in the input queue or there is an error
				...
				}else if (isNumber(temp) || onlyLetter(temp)){				//if the input is a number or is only consist of letters
			...
			if (error == true) {							//if there is an error, print "Error! " and clear the output queue
				...
				// assignment 4
				Stack eval = new Stack();				//create a stack to hold result of "eval*" expression
				while (true) {
					String temp = output.dequeue(); 		//get the elements from output queue
					if ( temp == null) break;			//if this is no more element in the output queue, break the program
					else if (isOperator(temp)) {			//if the element from the output queue is an operator
						stepcount = stepcount+1;			//create a new step
						String operand2 = eval.pop();	//define operand2 to the top value of "eval" stack
						String operand1 = eval.pop();	//define operand1 to the top value of "eval" stack
						String eval_step = "<" + operand1 + temp + operand2 + ">";	//define eval_step to the form of "number 1 + operator + number 2"
						String head = "Eval" + Integer.toString(stepcount);			//define head to the form of "Eval + number"
						println ( head + ":" + eval_step);							//print the result in form of "Eval*: number1 + operator + number 2"
						eval.push(head);												//push "Eval*" into the eval stack
					}else {						//if temp is not an operator
						eval.push(temp);			//push it into the eval stack
					}
				}
			}
